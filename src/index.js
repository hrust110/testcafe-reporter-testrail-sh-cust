...
module.exports = function () {
    return {
        ...
        reportTaskStart (/* startTime, userAgents, testCount */) {
            ...
            if (reqParamsPresent) this.testrail = new Testrail.TestRail(process.env.TESTRAIL_HOST, process.env.TESTRAIL_USERNAME, process.env.TESTRAIL_APIKEY); else 
                console.error('Error. Expected all 3 TESTRAIL_HOST, TESTRAIL_USERNAME, TESTRAIL_APIKEY ' + 'in environment variables for the testrail-simple reporter to work'); // process.exit(1);
        ...
        async reportTestStart ( name, testMeta ) {
            ...
            if (testRailCaseId) {
                ...
                console.log('Did not find testRailCaseId in the testMeta. ' + 'Please add {testRailCaseId: "CXXXXXX"} in this format to the testMeta to ' + 'upload results to TestRail');
        },
            ...
            if (Object.keys(this.testResults).length > 0) {
                ...
                if (Object.values(this.screenshotsToCaseIdMap).length > 0) {
                    if (!process.env.SKIP_UPLOAD_SCREENSHOTS) {
                        ...
                        console.log('Found screenshots in the executed tests, consider removing the env variable ' + 'SKIP_UPLOAD_SCREENSHOTS to upload screenshots to test run results');
        ...
        async checkNCreateTestRuns () {
            ...
            for (const results of Object.values(this.testResults)) {
                ...
                const statusId = results['status_id']; // Check if new run is required for this suite/project combo
                ...
                if (!this.projectRuns[projectId][suiteId]) {
                    const runId = await this.createNewTestRun(projectId, suiteId);
